...
<div class="container-fluid">
	<div class="row-fluid">
			<div class="span12" id="displayWall">
				<hr />
				<h4 class="success">你个人的{$month}月{$day}日墙</h4>
				<div class="result">
					...
					{/if}
						...
						<div class="resultTextData" data="{$item.twitter_id}">
					   		<div class="textData">
					   			<h6><span><a href="{#public#}friend/findfriendmainpage?id=96">Days366</a>&nbsp;的收藏</span></h6>
					   			<hr />
					...
					<h4>{$item.twitter_title}</h4>
		   			...
		   			{/if}
					   			<div class="buttons">
									<span class="dayLike"><button disabled class="btn btn-mini" title="喜欢"><i class="icon-heart"></i>{$item.twitter_like}</button></span>
									<span class="dayFavourite"><button disabled class="btn btn-mini" title="收藏"><i class="icon-plus-sign"></i>{$item.twitter_favourite}</button></span>
									<span class="dayComment"><a class="btn btn-mini"  href="{#public#}message/find?type=twitter&id={$item.twitter_id}" title="评论" ><i class="icon-comment"></i>{$item.twitter_comment}</a></span>
							</div>
							</div>		   	
						</div>
					</div>
